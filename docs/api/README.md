...
[momo SCM OpenAPI 3.1 規格檔](./momo-scm.openapi.yaml) 整合了全部 19 份 momo SCM 供應商規範檔案，包含 110 個實體 HTTP 請求端點以及 47 個邏輯動作（例如多個功能共用 `OrderServlet.do` 並透過 `doAction` 區分）。
...
[mo店+ OpenAPI 3.1 規格檔](./mo-store-plus.openapi.yaml) 收錄了 32 個已說明的 API 操作端點。

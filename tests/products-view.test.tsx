...
test("點擊修改會開啟對話框並預先填入該商品現值", async () => {
...
test("儲存變更會帶著商品 id 與新值呼叫 updateProduct，並重新載入清單", async () => {
  ...
  const dialog = await screen.findByRole("dialog");
  const nameField = within(dialog).getByLabelText(/商品名稱/);
  await user.clear(nameField);
  await user.type(nameField, "新名稱");
  const stockField = within(dialog).getByLabelText(/庫存/);
